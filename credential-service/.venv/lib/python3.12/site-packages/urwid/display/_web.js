...
function handle_recv() {
  ...
  if (conn.status == 403 && update_method == 'polling') {
    ...
    alert(
      'This server does not allow polling clients.\n\n' +
  ...
  if (conn.status == 503) {
    set_status('Connection Failed');
    alert('The server has reached its maximum number of ' + 'connections.\n\nPlease try again later.');
    return;
  }
  if (conn.status != 200) {
    set_status('Connection Failed');
...
function do_send() {
  ...
  if (send_queue_out > tmp_send_queue_in) {
    ...
    out = send_queue.slice(send_queue_out, tmp_send_queue_in).join('\n');
  }
